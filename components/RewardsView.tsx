  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {sortedRewards.map(reward => (
        <div key={reward.id} className="bg-tg-secondary-bg p-4 rounded-xl flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-grow min-w-0">
            <span className="text-4xl">{reward.icon || '🎁'}</span>
            <div className="flex-grow">
              <h3 className="font-bold text-tg-text truncate">{reward.name}</h3>
              <p className="text-sm text-tg-hint truncate">{reward.description}</p>
            </div>
          </div>
          <button
            onClick={() => onBuyReward(reward)}
            disabled={(currentUser.points || 0) < reward.cost}
            className="bg-tg-button text-tg-button-text font-bold py-2 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 disabled:bg-gray-500 disabled:scale-100 disabled:cursor-not-allowed flex-shrink-0"
          >
